...
public class ConfigManager {
    ...
    public static void writeCEMItems(){
        for (String rp : PackManager.getPacks()) {
            ...
            if (hasCEMFolder(pathToResourcePacks + rp)) {
                MYLOGGER.info(rp + " resource pack has a CEM folder!");
                CEMWriter.writeConfig(new File(pathToResourcePacks + rp), rp.endsWith(".zip"), false);
            }
        }
    }
    ...
    public static boolean hasCEMFolder(String path) {
        Path fpath = Paths.get(path.replace(".zip", "") + pathToCEMFolder);

        if(path.endsWith(".zip")) return hasZipCEMFolder(Paths.get(path));
    ...
    public static boolean hasZipCEMFolder(Path path) {
        try (FileSystem zipFS = FileSystems.newFileSystem(path, (ClassLoader) null)) {
            Path citFolder = zipFS.getPath(pathToCEMFolder);
            return Files.isDirectory(citFolder);
        } catch (IOException e) {
            e.printStackTrace();
            return false;
