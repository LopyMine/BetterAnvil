...
public class Painters {
    ...
    public void renderOrderedTextTooltip(MatrixStack matrices, List<OrderedText> lists, List<ItemStack> items, int x, int y, Screen screen, TextRenderer textRenderer, ItemRenderer itemRenderer, boolean showItems){
        ...
        this.renderTooltipFromComponents(matrices, components,x,y,screen,textRenderer,itemRenderer);
    }

    public void renderTooltipFromComponents(MatrixStack matrices, List<TooltipComponent> components, int x, int y, Screen screen, TextRenderer textRenderer, ItemRenderer itemRenderer) {
        if (!components.isEmpty()) {
            ...
            for(r = 0; r < components.size(); ++r) {
                tooltipComponent2 = (TooltipComponent)components.get(r);
                tooltipComponent2.drawItems(textRenderer, n+4, q+4, matrices, itemRenderer,400);
                q += tooltipComponent2.getHeight() + (r == 0 ? 2 : 0);
            }
