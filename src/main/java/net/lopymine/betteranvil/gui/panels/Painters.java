...
public class Painters {
    ...
    public void renderTextTooltip(DrawContext context, List<Text> list, List<ItemStack> items, int x, int y, Screen screen, TextRenderer textRenderer, ItemRenderer itemRenderer, boolean showItems){
        this.renderOrderedTextTooltip(context, Lists.transform(list, Text::asOrderedText),items,x,y,screen,textRenderer,itemRenderer, showItems);
    }
    public void renderOrderedTextTooltip(DrawContext context, List<OrderedText> lists, List<ItemStack> items, int x, int y, Screen screen, TextRenderer textRenderer, ItemRenderer itemRenderer, boolean showItems){
        ...
        this.renderTooltipFromComponents(context, components,x,y, HoveredTooltipPositioner.INSTANCE,screen,textRenderer,itemRenderer);
    }

    public void renderTooltipFromComponents(DrawContext context, List<TooltipComponent> components, int x, int y, TooltipPositioner positioner, Screen screen, TextRenderer textRenderer, ItemRenderer itemRenderer) {
        if (!components.isEmpty()) {
            ...
            Vector2ic vector2ic = positioner.getPosition(screen.width,screen.height,x, y, i, j);
            int n = vector2ic.x();
            int o = vector2ic.y();
            ...
            panel.setSize(i,j);
            tooltip.paintBackground(context,n,o,panel);
            ...
            for(r = 0; r < components.size(); ++r) {
                tooltipComponent2 = (TooltipComponent)components.get(r);
                tooltipComponent2.drawItems(textRenderer, n+4, q+4,context);
                q += tooltipComponent2.getHeight() + (r == 0 ? 2 : 0);
            }
