...
public class WDroppedItem extends WWidget {
    ...
    private void renderItem(MatrixStack matrices, float tick) {
        ...
        float m = model.getTransformation().getTransformation(ModelTransformation.Mode.GROUND).scale.getY();
        ...
        float o = model.getTransformation().ground.scale.getX();
        float p = model.getTransformation().ground.scale.getY();
        float q = model.getTransformation().ground.scale.getZ();
        float s;
        float t;
        if (!bl) {
